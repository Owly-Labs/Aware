...
public actor TestRunner {
    ...
    public func run(options: RunOptions) async -> TestResult {
        ...
        SwiftAware.logger.info("Starting test run", metadata: [
        ...
        for test in testsToRun {
            let testStart = Date()
            SwiftAware.logger.testStarted(test.name, tier: test.tier)
            ...
            if status == .passed {
                passed += 1
                SwiftAware.logger.testPassed(test.name, duration: duration)
            } else {
                failed += 1
                SwiftAware.logger.testFailed(test.name, error: testError?.message ?? "Unknown error")
