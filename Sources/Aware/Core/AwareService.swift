...
public final class Aware: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.aware.framework", category: "Aware")

    // MARK: - State Registry

    /// Registered views and their properties
    @Published private var viewRegistry: [String: AwareViewSnapshot] = [:]
    ...
    private var gestureCallbacks: [String: [AwareGestureType: AwareGestureCallback]] = [:]

    /// Parameterized gesture callbacks (for gestures that need direction, scale, etc.)
    private var parameterizedGestureCallbacks: [String: [AwareGestureType: AwareParameterizedGestureCallback]] = [:]

    /// Text bindings by view ID for direct text manipulation
    private var textBindings: [String: AwareTextBinding] = [:]
    ...
    /// Execute an action from command
    ...
    public func registerView(
        ...
        if viewRegistry[id] == nil {
            ...
            viewRegistry[id]?.isVisible = true
            if let label = label { viewRegistry[id]?.label = label }
            if let parentId = parentId { viewRegistry[id]?.parentId = parentId }
        }

        // Add to parent's children (deduplicated)
        if let parentId = parentId {
            parentChildMap[id] = parentId
            if viewRegistry[parentId]?.childIds.contains(id) == false {
                viewRegistry[parentId]?.childIds.append(id)
            }
        }
    ...
    public func registerAnimation(_ viewId: String, animation: AwareAnimationState) {
        viewRegistry[viewId]?.animation = animation
        scheduleSnapshotWrite()
    }

    /// Clear animation state
    public func clearAnimation(_ viewId: String) {
        viewRegistry[viewId]?.animation = nil
        scheduleSnapshotWrite()
    }
    ...
    public func registerAction(_ viewId: String, action: AwareActionMetadata) {
        viewRegistry[viewId]?.action = action
        scheduleSnapshotWrite()
    }
    ...
    public func registerBehavior(_ viewId: String, behavior: AwareBehaviorMetadata) {
        viewRegistry[viewId]?.behavior = behavior
        scheduleSnapshotWrite()
    }
    ...
    public func updateScrollState(
        ...
        guard var snapshot = viewRegistry[viewId] else { return }
    ...
    public func describeView(_ viewId: String) -> AwareViewDescription? {
        guard let snapshot = viewRegistry[viewId] else { return nil }
    ...
    public var visibleViewCount: Int {
        viewRegistry.values.filter { $0.isVisible }.count
    }

    // MARK: - Tree Building

    private func buildViewTree(from views: [String: AwareViewSnapshot], maxDepth: Int) -> [AwareViewNode] {
        // Find root nodes (no parent or parent not in views)
        let rootIds = views.keys.filter { id in
            guard let snapshot = views[id] else { return false }
    ...
    private func buildNode(id: String, views: [String: AwareViewSnapshot], depth: Int, maxDepth: Int) -> AwareViewNode? {
        guard depth < maxDepth, let snapshot = views[id] else { return nil }
    ...
    /// Execute a gesture on a view (iOS callback-based)
    @discardableResult
    ...
    /// Set text directly via binding (iOS/macOS)
    @discardableResult
    ...
    /// Focus a view (delegates to AwareFocusManager)
    @discardableResult
    ...
    /// Navigate back (delegates to AwareNavigationManager)
    @discardableResult
    ...
    /// Direct tap - invokes registered action callback WITHOUT moving user's mouse
    @discardableResult
    ...
    public func assertTextContains(_ viewId: String, substring: String) -> AwareAssertionResult {
        guard let snapshot = viewRegistry[viewId],
              let text = snapshot.visual?.text else {
