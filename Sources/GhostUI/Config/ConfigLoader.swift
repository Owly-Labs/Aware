...
public struct ConfigLoader {
    ...
    public static func load(from path: String? = nil) -> AwareConfig {
        do {
            ...
            #if DEBUG
            print("[GhostUI] Failed to load config: \(error.localizedDescription), using defaults")
            #endif
            return AwareConfig()
        }
    }
    ...
    public static func loadAndValidate(from path: String? = nil) throws -> AwareConfig {
        ...
        // Search in bundle
        if let bundleURL = Bundle.main.url(forResource: ".ghostui", withExtension: "json") {
