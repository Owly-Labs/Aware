...
public enum CodeGenerator {
    ...
    public static func generateEntryLog(
        ...
        if verbosity == "verbose" && !parameters.isEmpty {
            ...
            return "SwiftAware.shared.logger.debug(\"[ENTER] \(functionName)(\(paramStr))\"\(categoryArg))"
        } else {
            return "SwiftAware.shared.logger.debug(\"[ENTER] \(functionName)\"\(categoryArg))"
        }
    }
    ...
    public static func generateExitLog(
        ...
        if hasReturnValue {
            return "SwiftAware.shared.logger.debug(\"[EXIT] \(functionName) -> \\(type(of: _result)) (\(durationStr)s)\"\(categoryArg))"
        } else {
            return "SwiftAware.shared.logger.debug(\"[EXIT] \(functionName) (\(durationStr)s)\"\(categoryArg))"
        }
    }
    ...
    public static func generateErrorLog(
        ...
        let categoryArg = category.map { ", category: \"\($0)\"" } ?? ""
        let durationStr = "\\(String(format: \\\"%.3f\\\", _duration))"
        return "SwiftAware.shared.logger.error(\"[ERROR] \(functionName): \\(_error) (\(durationStr)s)\"\(categoryArg))"
    }
}
