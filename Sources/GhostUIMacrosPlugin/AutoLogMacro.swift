...
public struct AutoLogMacro: PeerMacro {
    ...
    private static func generateLoggedWrapper(
        ...
        case "verbose":
            ...
            entryLog = "GhostUI.shared.logger.debug(\"[ENTER] \(originalName)(\(paramStr))\"\(categoryArg))"
        default: // standard
            entryLog = "GhostUI.shared.logger.debug(\"[ENTER] \(originalName)\"\(categoryArg))"
        }
        ...
        default:
            if functionInfo.returnType != nil {
                exitLog = "GhostUI.shared.logger.debug(\"[EXIT] \(originalName) -> \\(type(of: _result)) (\\(_duration)s)\"\(categoryArg))"
            } else {
                exitLog = "GhostUI.shared.logger.debug(\"[EXIT] \(originalName) (\\(_duration)s)\"\(categoryArg))"
            }
        }

        // Build error log
        let errorLog = "GhostUI.shared.logger.error(\"[ERROR] \(originalName): \\(_error) (\\(_duration)s)\"\(categoryArg))"
