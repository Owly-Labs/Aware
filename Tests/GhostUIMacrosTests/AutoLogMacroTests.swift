...
#if canImport(GhostUIMacrosPlugin)
import GhostUIMacrosPlugin
        ...
        assertMacroExpansion(
            ...
            class UserService {
                ...
                func _logged_getUser() {    let _start = CFAbsoluteTimeGetCurrent()
                    GhostUI.shared.logger.debug("[ENTER] getUser", category: "Services")
                    getUser()
                    let _duration = CFAbsoluteTimeGetCurrent() - _start
                    GhostUI.shared.logger.debug("[EXIT] getUser (\\(_duration)s)", category: "Services")
                }
            }
            """,
            macros: testMacros,
        ...
        assertMacroExpansion(
            ...
            struct AuthManager {
                ...
                func _logged_login() {    let _start = CFAbsoluteTimeGetCurrent()
                    GhostUI.shared.logger.debug("[ENTER] login", category: "Managers")
                    login()
                    let _duration = CFAbsoluteTimeGetCurrent() - _start
                    GhostUI.shared.logger.debug("[EXIT] login (\\(_duration)s)", category: "Managers")
                }
            }
            """,
            macros: testMacros,
        ...
        assertMacroExpansion(
            ...
            struct MyStruct {
                ...
                func _logged_doSomething() {    let _start = CFAbsoluteTimeGetCurrent()
                    GhostUI.shared.logger.debug("[ENTER] doSomething", category: "CustomCategory")
                    doSomething()
                    let _duration = CFAbsoluteTimeGetCurrent() - _start
                    GhostUI.shared.logger.debug("[EXIT] doSomething (\\(_duration)s)", category: "CustomCategory")
                }
            }
            """,
            macros: testMacros,
        ...
        assertMacroExpansion(
            ...
            class DataProvider {
                ...
                func _logged_fetch() {    let _start = CFAbsoluteTimeGetCurrent()
                    GhostUI.shared.logger.debug("[ENTER] fetch()", category: "Providers")
                    fetch()
                    let _duration = CFAbsoluteTimeGetCurrent() - _start
                    GhostUI.shared.logger.debug("[EXIT] fetch (\\(_duration)s)", category: "Providers")
                }
            }
            """,
            macros: testMacros,
        ...
        assertMacroExpansion(
            ...
            class MyCache {
                ...
                func _logged_refresh() {    let _start = CFAbsoluteTimeGetCurrent()
                    GhostUI.shared.logger.debug("[ENTER] refresh", category: "Caches")
                    refresh()
                    let _duration = CFAbsoluteTimeGetCurrent() - _start
                    GhostUI.shared.logger.debug("[EXIT] refresh (\\(_duration)s)", category: "Caches")
                }
            }
            """,
            macros: testMacros,
        ...
        assertMacroExpansion(
            ...
            class MyService {
                ...
                func _logged_loggedMethod() {    let _start = CFAbsoluteTimeGetCurrent()
                    GhostUI.shared.logger.debug("[ENTER] loggedMethod", category: "Services")
                    loggedMethod()
                    let _duration = CFAbsoluteTimeGetCurrent() - _start
                    GhostUI.shared.logger.debug("[EXIT] loggedMethod (\\(_duration)s)", category: "Services")
                }
                func excludedMethod() {}
            }
            """,
