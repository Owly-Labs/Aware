...
```
Sources/
  SwiftAware/               Main library (33 files)
    ...
    Config/                 Configuration
      ...
      ConfigLoader          Loads .swiftaware.json from disk or bundle
  ...
  SwiftAwareMacrosPlugin/   Compiler plugin
    ...
    Plugin                  SwiftAwareMacrosPlugin entry point
Tests/
  SwiftAwareTests/          4 tests — logger, config, presets, tiers
  SwiftAwareMacrosTests/    7 tests — macro expansion, category inference, @NoLog exclusion
```
...
| `Aware.validate()` | Validate .swiftaware.json configuration |
...
Apps configure via `.swiftaware.json` (project root or app bundle). Searched filenames: `.swiftaware.json`, `swiftaware.json`, `.swiftaware.config.json`.
