...
public final class AwareFrameTracker {
    ...
    public func getFrame(_ viewId: String) async -> CGRect? {
        guard
            let xStr = await Aware.shared.getState(viewId, key: "frame.x"),
            let yStr = await Aware.shared.getState(viewId, key: "frame.y"),
            let widthStr = await Aware.shared.getState(viewId, key: "frame.width"),
            let heightStr = await Aware.shared.getState(viewId, key: "frame.height"),
            let x = Double(xStr),
            let y = Double(yStr),
